...
CREATE TABLE IF NOT EXISTS users (
  ...
  flagged INTEGER NOT NULL DEFAULT 0, agent_status TEXT NOT NULL DEFAULT 'offline', custom_fields TEXT NOT NULL DEFAULT '{}', privacy TEXT NOT NULL DEFAULT '{}',
...
CREATE TABLE IF NOT EXISTS statuses (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, body TEXT NOT NULL, media_id INTEGER, created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')));
...
const ready = (async () => {
  ...
  const stcols = db.engine === 'sqlite' ? (await db.prepare('PRAGMA table_info(statuses)').all()).map((c) => c.name) : [];
  if (db.engine === 'sqlite' && !stcols.includes('media_id')) await db.exec('ALTER TABLE statuses ADD COLUMN media_id INTEGER');
  const cvcols = db.engine === 'sqlite' ? (await db.prepare('PRAGMA table_info(conversations)').all()).map((c) => c.name) : [];
  if (db.engine === 'sqlite' && !cvcols.includes('disappearing')) await db.exec('ALTER TABLE conversations ADD COLUMN disappearing INTEGER NOT NULL DEFAULT 0');
  if (db.engine === 'sqlite' && !ucols3.includes('privacy')) await db.exec("ALTER TABLE users ADD COLUMN privacy TEXT NOT NULL DEFAULT '{}'");
  await db.exec(`
...
/* ---------- helpers ---------- */
const USER_FIELDS = 'id, username, display_name, phone, about, avatar_text, avatar_color, avatar_url, is_bot, is_official, is_admin, role, verified, title, blocked, flagged, agent_status, privacy, created_at';
const parseJSON = (s, d = {}) => { try { return JSON.parse(s || null) ?? d; } catch { return d; } };
...
function publicUser(row) {
  ...
  return {
    ...
    blocked: !!row.blocked, flagged: !!row.flagged, agentStatus: row.agent_status || 'offline',
    privacy: parseJSON(row.privacy), createdAt: row.created_at,
  };
}
...
/* ---------- updates / status ---------- */
async function addStatus(userId, body, mediaId = null) {
  const info = await db.prepare('INSERT INTO statuses (user_id, body, media_id) VALUES (?, ?, ?)').run(userId, body, mediaId);
...
async function updateOwnProfile(userId, patch) {
  const sets = []; const params = [];
  if (patch.privacy !== undefined) { sets.push('privacy = ?'); params.push(JSON.stringify(patch.privacy || {})); }
...
/* ---------- pesan sementara, keluar grup, galeri, hapus akun ---------- */
async function setDisappearing(conversationId, seconds) {
  await db.prepare('UPDATE conversations SET disappearing = ? WHERE id = ?').run(Number(seconds) || 0, conversationId);
}
async function purgeExpired() {
  const rows = await db.prepare('SELECT id, disappearing FROM conversations WHERE disappearing > 0').all();
  let n = 0;
  for (const c of rows) {
    const r = await db.prepare("DELETE FROM messages WHERE conversation_id = ? AND created_at < strftime('%Y-%m-%dT%H:%M:%fZ','now',?)").run(c.id, `-${c.disappearing} seconds`);
    n += Number(r.changes || 0);
  }
  return n;
}
async function leaveConversation(userId, conversationId) {
  await db.prepare('DELETE FROM conversation_members WHERE user_id = ? AND conversation_id = ?').run(userId, conversationId);
  const left = (await db.prepare('SELECT COUNT(*) AS n FROM conversation_members WHERE conversation_id = ?').get(conversationId)).n;
  if (left === 0) {
    await db.prepare('DELETE FROM messages WHERE conversation_id = ?').run(conversationId);
    await db.prepare('DELETE FROM conversations WHERE id = ?').run(conversationId);
    return { gone: true };
  }
  return { gone: false };
}
async function listMediaMessages(conversationId) {
  return db.prepare(`SELECT m.id, m.body, m.created_at, m.media_id, md.mime AS media_mime, u.display_name AS sender_name
    FROM messages m JOIN media md ON md.id = m.media_id JOIN users u ON u.id = m.sender_id
    WHERE m.conversation_id = ? ORDER BY m.id DESC LIMIT 100`).all(conversationId);
}
async function deleteOwnAccount(userId) {
  await db.tx(async (q) => {
    await q.run('DELETE FROM messages WHERE sender_id = ?', userId);
    await q.run('DELETE FROM conversation_members WHERE user_id = ?', userId);
    await q.run('DELETE FROM sessions WHERE user_id = ?', userId);
    await q.run('DELETE FROM users WHERE id = ?', userId);
    await q.run(`DELETE FROM messages WHERE conversation_id IN
      (SELECT c.id FROM conversations c LEFT JOIN conversation_members cm ON cm.conversation_id = c.id WHERE cm.conversation_id IS NULL)`);
    await q.run(`DELETE FROM conversations WHERE id NOT IN (SELECT conversation_id FROM conversation_members)`);
  });
...
module.exports = {
  ...
  setDisappearing, purgeExpired, leaveConversation, listMediaMessages, deleteOwnAccount,
