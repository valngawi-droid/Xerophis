...
CREATE TABLE IF NOT EXISTS users (
  ...
  avatar_text TEXT NOT NULL, avatar_color TEXT NOT NULL DEFAULT '#7a1216', is_bot INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS sessions (token TEXT PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')));
...
function publicUser(row) {
  ...
  return {
    ...
    avatarText: row.avatar_text, avatarColor: row.avatar_color, isBot: !!row.is_bot, isOfficial: !!row.is_official,
...
async function createSession(userId, token) { await db.prepare('INSERT INTO sessions (token, user_id) VALUES (?, ?)').run(token, userId); }
...
async function insertMessage({ conversationId, senderId, body, kind, buttons, mediaId, replyTo, forwarded }) {
  const info = await db.prepare('INSERT INTO messages (conversation_id, sender_id, body, kind, buttons, media_id, reply_to, forwarded) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
...
async function getMessage(id) {
  const row = await db.prepare(`SELECT m.*, u.display_name AS sender_name FROM messages m JOIN users u ON u.id = m.sender_id WHERE m.id = ?`).get(id);
  if (!row) return null;
  return msgShape(row, await reactionsMapFor([row.id]));
}
async function listMessages(conversationId, limit = 200) {
  const rows = await db.prepare(`SELECT m.*, u.display_name AS sender_name FROM messages m JOIN users u ON u.id = m.sender_id
